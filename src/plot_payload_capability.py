from libs.payload_capability import func_maximum_payload, solve_maximum_payload
...
M_i_list = [
    first_stage_weight + second_stage_weight + third_stage_weight + PLF_weight,
    second_stage_weight + third_stage_weight,
    third_stage_weight   
]
M_f_list = [
    first_stage_weight - first_stage_prop + second_stage_weight + third_stage_weight + PLF_weight,
    second_stage_weight - second_stage_prop + third_stage_weight,
    third_stage_weight - third_stage_prop  
]

C3_list = np.linspace(-50, 20, 1000) # [km^2/s^2]
M_L_list = []
for C3 in C3_list:
    maximum_payload = solve_maximum_payload(Isp_list=Isp_list, M_i_list=M_i_list, M_f_list=M_f_list, C3=C3, radius_parking=200 + 6371)
