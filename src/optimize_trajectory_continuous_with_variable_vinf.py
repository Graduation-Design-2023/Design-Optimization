...
xl_array = np.array([js_launch0, tf0, -v_inf, -v_inf, -v_inf])
xu_array = np.array([js_launch1, tf1, v_inf, v_inf, v_inf])
...
class MyProblem(Problem):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        g1 = np.zeros(n)
        for i in range(0, n):
            v_inf_vec = np.array([[X[i,2], X[i,3], X[i,4]]]).T
            self.my_bvp.set_vals(X[i,0], X[i,1], v_inf_vec)
            f1[i] = self.my_bvp.solve(plot_is_enabled=False)
            f2[i] = X[i,1]
            g1[i] = X[i,2]**2 + X[i,3]**2 +  X[i,4]**2 - v_inf**2
...
class Bvp():
    def set_vals(self, js_start, tf, v_inf_vec):
        self.r0, ve = earth.position_JS(js_start)
        self.v0 = ve + v_inf_vec
...
if __name__ == '__main__':
    ...
    X  = res.X
    i = 0
    v_inf_vec = np.array([X[i,2], X[i,3], X[i,4]])
    my_bvp = Bvp()
    my_bvp.set_vals(X[i,0], X[i,1], v_inf_vec)
