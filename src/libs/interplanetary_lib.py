...
class LambertSolver():
    ...
    def calc_a(self):
        ...
        a = 1.5 * self.a_m
        n = 0
        while(np.abs(self.calc_delta_t(a) - self.delta_t) > self.delta_t * 10**(-5)):
            # print(a)
            rho = 0.5
...
class PlanetsTransOrbit():
    ...
    def trajectory_with_swingby(self, target_planet_list, arrival_JS_list, v_inf_in_list, theta_list, h_list, JS_tcm_list, plot_is_enabled = False):
        """
        swingbyを含む軌道を生成する
        引数---------------------------
        target_planet_list : n+2 list
            目標天体とスウィングバイ対象天体のリスト（[目標天体, n回目ののスウィングバイ対象, n-1回目のスウィングバイ対象, ..., 1回目のスウィングバイ対象, 地球]）
        arrival_time_list : n+2 list
            目標天体とスウィングバイ対象天体到着時の時刻JS（[目標天体, n回目のスウィングバイ対象, n-1回目のスウィングバイ対象, ..., 地球]）
        v_inf_in_list : n+1 list
        ...
        if (plot_is_enabled):
            ...
            ax1.set_xlim([-3.5 * 10**8,3.5 * 10**8])
            ax1.set_ylim([-3.5 * 10**8,3.5 * 10**8])
            ax1.set_zlim([-3.5 * 10**8,3.5 * 10**8])
            ax2 = fig.add_subplot(122)
            ax2.set_xlim([-3.5 * 10**8,3.5 * 10**8])
            ax2.set_ylim([-3.5 * 10**8,3.5 * 10**8])
        """
        ...
        for i in range(n+1):
            ...
            if (i != n):
                ...
                delta_v_swingby_list[i] = np.linalg.norm(v_vec_aft_i_1 - v_vec_aft_i_1_bfr_tcm)
    ...
    def trajectory_insertion01(self,theta,r_h,v_inf_vec,JS,r_a_planetary,oe_observation):
        ...
        if (r_a_planetary > r_h):# is perigee
            nu1_start = 0
            r_p1_vec = r_01_vec
            v_p1_vec = v1_start_vec
        else:# is apogee
            nu1_start = 180
            r_p1_vec = a1 * (1 - e1) * P_hat1_vec
            v_p1_vec = (self.planet_end.mu * (1 + e1) / a1 / (1 - e1))**0.5 * Q_hat1_vec

        r_12_vec , v1_end_vec, nu1_end = self.calc_point_where_change_plane(oe_observation, r_p1_vec, v_p1_vec, JS0_end)
        ...
        return r_01_vec, r_12_vec, v0_end_vec, v1_start_vec, v1_end_vec, nu1_start, nu1_end, JS0_end, JS1_end
    
    def calc_point_where_change_plane(self, oe_observation, r_p1_vec, v_p1_vec, JS0_end):
        """
        ...
        nu_n
        """
        a1, e1, i1, omega1, Omega1, t_p1, P_hat1_vec, Q_hat1_vec, W_hat1_vec = self.calculator_end.calc_orbital_elems_from_r_v(r_p1_vec, v_p1_vec, JS0_end)
        p1 = a1 * (1 - e1**2)
        _, _, i3, omega3, Omega3, _ = oe_observation
        ...
        if (i1_rad == 0):
            r_n_hat_vec = np.array([[np.cos(Omega3_rad)], [np.sin(Omega3_rad)], [0]])
        elif (i3_rad == 0):
            r_n_hat_vec = np.array([[np.cos(Omega1_rad)], [np.sin(Omega1_rad)], [0]])
        elif(Omega1_rad == Omega3_rad):
            r_n_hat_vec = np.array([[np.cos(Omega1_rad)], [np.sin(Omega1_rad)], [0]])
        else:
        ...
        r_01 = np.linalg.norm(r_p1_vec)
        cos_nu_n = np.dot(e_n_vec.T, r_p1_vec) / r_01
    ...
    def trajectory_insertion12(self, oe_observation, r_12_vec, JS1_end, target_is_perigee):
        ...
        if (x == 0):
            nu2_start_rad = np.pi / 2 * y / np.abs(y)
        else:
    ...
    def trajectory_insertion(self, theta, r_h, v_in_vec, JS0_start, r_a_planetary, oe_observation, target_is_perigee, plot_is_enabled = True):
        ...
        if (plot_is_enabled):
            ...
            self.calculator_end.plot_trajectory(r_23_vec, v2_end_vec, JS2_end, nu2_start, nu2_end, ax, 'r')
    ...
    def trajectory_insertion_2sats(self, theta, r_h, v_in_vec, JS0_start, r_a_planetary, oe_observation_d, oe_observation_p, target_is_perigee, plot_is_enabled = True):
        # 親機子機一緒
        ...
        delta_v01 = np.linalg.norm(v0_end_vec - v1_start_vec)
        delta_v12 = np.linalg.norm(v1_end_vec - v2_start_vec)
        delta_v12_p = np.linalg.norm(v1_end_vec_p - v2_start_vec_p)
        delta_v23_p = np.linalg.norm(v2_end_vec_p - v3_start_vec_p)
        delta_v23_d = np.linalg.norm(v2_end_vec - v3_start_vec_d)
